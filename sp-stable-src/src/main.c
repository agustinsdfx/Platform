...
int main(void) {
	...
	while (!WindowShouldClose()) {
		...
		//gearRotationAngle += GEAR_ROTATION_SPEED * dt;
		//if (gearRotationAngle >= 360.0f) gearRotationAngle -= 360.0f;
		...
		if (!hideUI) {
			...
			if (showStats) {
				DrawText("Version: 1.2 OFFICIAL RELEASE | Release vID 29.12.2025", 10, screenHeight - 60, 20, BLUE);
				DrawText("Desarrollado por AGUSTIN_SDFX | Build x64 C lang & C++ lang - SDFX Engine - OpenGL - SDL", 10, screenHeight - 35, 10, BLUE);
			}
