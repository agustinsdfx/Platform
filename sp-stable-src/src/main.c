...
void AddConsoleLog(const char* text) {
...
void ResetPlayer(Player *player, Block startPlatform) {
...
void ResetGame(Player *player, Block *baseBlocks) {
...
void DrawBlockShape(Block b) {
    if (b.shape >= SHAPE_CUST1 && b.shape <= SHAPE_CUST6) {
    ...
    switch (b.shape) {
        ...
        case SHAPE_TRIANGLE: 
            DrawTriangle(
                (Vector2){b.rect.x + b.rect.width/2, b.rect.y},
                (Vector2){b.rect.x, b.rect.y + b.rect.height},
                (Vector2){b.rect.x + b.rect.width, b.rect.y + b.rect.height},
                b.color
            );
            break;
        case SHAPE_CIRCLE: 
        ...
        case SHAPE_RHOMBUS:
             DrawTriangle(
                (Vector2){b.rect.x + b.rect.width/2, b.rect.y},
                (Vector2){b.rect.x, b.rect.y + b.rect.height/2},
                (Vector2){b.rect.x + b.rect.width, b.rect.y + b.rect.height/2},
                b.color
            );
            DrawTriangle(
                (Vector2){b.rect.x, b.rect.y + b.rect.height/2},
                (Vector2){b.rect.x + b.rect.width/2, b.rect.y + b.rect.height},
                (Vector2){b.rect.x + b.rect.width, b.rect.y + b.rect.height/2},
                b.color
            );
            break;
...
void DrawPlayer(Player p, Color color) {
    if (hasPlayerTexture) {
...
void DrawTextRight(const char* text, int y, int fontSize, Color color, int screenWidth) {
    int textWidth = MeasureText(text, fontSize);
    DrawText(text, screenWidth - textWidth - 10, y, fontSize, color);
}

void SaveGame(const Player *player, const Block *blocks, int isNight, WeatherType weather, int playerColorIndex, int selectedColorIndex, int selectedShapeIndex) {
    GameData data = { 0 };
...
void LoadGame(Player *player, Block *blocks, int *isNight, WeatherType *weather, int *playerColorIndex, int *selectedColorIndex, int *selectedShapeIndex) {
...
int main(void) {
    ...
    InitWindow(screenWidth, screenHeight, "Platform [Release] - [SDFX Engine2D] [OpenGL 3.3]");
    InitAudioDevice();
    ...
    while (!WindowShouldClose()) {
        ...
        if (!gamePaused) {
            ...
            if (!showConsole && !showCheatUI) {
                ...
                if (wheel != 0) {
                    ...
                    if (selectedShapeIndex < 0) selectedShapeIndex = 10;
                    if (selectedShapeIndex > 10) selectedShapeIndex = 0;
                    previewTimer = 3.0f;
                }
        ...
        BeginDrawing();
            ...
            BeginMode2D(camera);
                for (int i = 0; i < MAX_BLOCKS; i++) {
                    if (blocks[i].active) DrawBlockShape(blocks[i]);
                }
            ...
            if (!hideUI) {
                ...
                if (showControls) {
                    ...
                    int textX = boxX + 20;
                    int textY = boxY + 20;
                    DrawText("CONTROLES", textX, textY, 20, YELLOW);
                    DrawText("P: PAUSAR JUEGO", textX, textY + 30, 10, RED);
                    DrawText("Flechas: Mover/Saltar", textX, textY + 50, 10, WHITE);
                    DrawText("Z: Cambiar Color Jugador", textX, textY + 70, 10, ORANGE);
                    DrawText("X: Reiniciar Juego", textX, textY + 90, 10, RED);
                    DrawText("C: Ocultar Interfaz", textX, textY + 110, 10, GRAY);
                    DrawText("Click Der: Poner Bloque", textX, textY + 130, 10, WHITE);
                    DrawText("Click Izq: Quitar Bloque", textX, textY + 150, 10, WHITE);
                    DrawText("Rueda: Cambiar Forma", textX, textY + 170, 10, WHITE);
                    DrawText("Click Central: Cambiar Color", textX, textY + 190, 10, WHITE);
                    DrawText("F2: Stats | F3: Debug", textX, textY + 210, 10, WHITE);
                    DrawText("F4: Dia/Noche | F5: Clima", textX, textY + 230, 10, WHITE);
                    DrawText("F6: Camara Toggle", textX, textY + 250, 10, WHITE);
                    DrawText("F7: Cambiar Musica", textX, textY + 270, 10, GREEN);
                    DrawText("F8: Guardar Partida", textX, textY + 290, 10, LIME);
                    DrawText("F9: Cargar Partida", textX, textY + 310, 10, LIME);
                    DrawText("F10: Ver Consola", textX, textY + 330, 10, VIOLET);
                    DrawText("G: Alternar Gear", textX, textY + 350, 10, PINK);
                    DrawText("F11: Pantalla Completa", textX, textY + 370, 10, WHITE);
                    DrawText("ESC: Salir del Juego", textX, textY + 390, 10, RED);
                    DrawText("K: Cheat Console", textX, textY + 410, 10, PURPLE);
                    DrawText("F12: Screenshot", textX, textY + 430, 10, WHITE);
                }

                if (showStats) {
                    DrawText("Version: 1.1 OFFICIAL RELEASE | Release vID 20.12.2025", 10, screenHeight - 60, 20, BLUE);
                    DrawText("Desarrollado por AGUSTIN_SDFX | Build x86_64 C lang - SDFX Engine2D - OpenGL 3.3", 10, screenHeight - 35, 10, BLUE);
                }
                ...
                if (showDebug) {
                    ...
                    DrawText(TextFormat("Gear [%d/6]: gear%d.png", currentGearIndex + 1, currentGearIndex + 1), 10, 200, 10, GRAY);
                }
            } 
